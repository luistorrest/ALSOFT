...
def actualizar_velocidad(event=None):
    ...
    velocidad_variador_str = int(text_velocidad_banda.get())
    
    if(velocidad_variador_str>40):
        if(velocidad_variador_str<=110):
            velocidad_variador = velocidad_variador_str
    else:
        text_velocidad_banda.delete("1.0", "end")
        #text_velocidad_banda.insert(0,velocidad_variador)
...
def escribir_PLC(contador,clase_codificada,boton_start_stop):
    ...
    #print(int(text_velocidad_banda.get("1.0", "end-1c")))
    with socket.socket(socket.AF_INET,socket.SOCK_STREAM) as plc:
        ...
        frame.append(velocidad_variador) # Velocidad baja del variador   -< VEl variador
...
#Se crea el textbox para ingresar la velocidad de la banda 
global text_velocidad_banda
text_velocidad_banda = tk.Entry(MainWindow)
text_velocidad_banda.pack()
text_velocidad_banda.bind("<Return>",actualizar_velocidad)
text_velocidad_banda.place(x = 700,y = 730)

# Label para el vrelocidad de la bamda
label_text_velocidad_banda = tk.Label(text='Velocidad banda', font = ("calibri", 12, "bold"), fg = '#AAAABA', bg = "#314354")
label_text_velocidad_banda.place(x=700,y =700)
